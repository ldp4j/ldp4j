...
final class ObjectAssertions {
	...
	public static <T extends Object> AbstractFilter<T> notNull() {
		return new AbstractFilter<T>() {
			@Override
			protected <F extends T> IExpectation<F> createExpectation(final F value) {
				return new Expectation<F>(value!=null, "Expected not null value",value);
			}
		};
	}
