...
/**
 * Base JAX-RS proxy implementation to <i>Linked Data Platform
 * Containers</i>.
 * 
 ...
 */
public abstract class AbstractRemoteLDPContainer implements IRemoteLDPContainer {

	private static final Logger LOGGER=LoggerFactory.getLogger(AbstractRemoteLDPContainer.class);
	 ...
	 * 
	 * @param target
	 *            The URL of the target container.
	 */
	public AbstractRemoteLDPContainer(URL target) {
		if(target==null) {
			throw new IllegalStateException("Object 'target' cannot be null");
		}
		this.target=target;
		if(LOGGER.isDebugEnabled()) {
			LOGGER.debug(String.format("Creating remote LDP container for target '%s'",target));
		}
	}
